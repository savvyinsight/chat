  ...
  return (
    <div className="max-w-md mx-auto mt-20 p-6 bg-white rounded shadow">
      ...
      <form onSubmit={submit} className="space-y-3">
        <input className="w-full p-2 border rounded" placeholder="email or phone" value={identifier} onChange={(e) => setIdentifier(e.target.value)} />
        <input className="w-full p-2 border rounded" placeholder="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
        <div className="flex justify-between items-center">
          <button className="px-4 py-2 bg-blue-600 text-white rounded" type="submit">Login</button>
        </div>
      </form>
      <p className="text-xs text-gray-500 mt-4">Login will use the backend JWT handler if available; otherwise it will accept user_id in response as fallback.</p>
    </div>
  )
}
